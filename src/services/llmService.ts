 ...
 */
export async function classifyWithLLM(userInput: string): Promise<DomainClassification> {
  try {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        ...
        'Cualquier otro tema (clima, matemáticas, programación, etc.) ' +
        'es OUT_OF_DOMAIN.\n\n' +
 ...
 */
export function buildMessagesFromMemory(messages: Message[]) {
  const formattedMessages = [new SystemMessage(SYSTEM_PROMPT)];
